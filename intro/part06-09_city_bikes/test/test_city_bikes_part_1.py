...
etaisyys(asemat, "Kaivopuisto", "Laivasillankatu")"""
    ...
    def test_5_distance_return_type(self):
        load_station_data = load(exercise, function1, 'fi')
        distance = load(exercise, function2, 'fi')
        stations = load_station_data("stations1.csv")
        val = distance(stations, "Kaivopuisto", "Laivasillankatu")
        code = """asemat = hae_asematiedot("stations1.csv")
etaisyys(asemat, "Kaivopuisto", "Laivasillankatu")"""
    ...
    def test_6_distance_works(self):
        for start, end, dist in [
            ...
            code = 'asemat = hae_asematiedot("stations1.csv")\n'+f'etaisyys(asemat, "{start}", "{end}")'

            self.assertTrue(close(val, dist), f"The answer {val} returned by function is wrond, it should be {dist} when called with\n{code}")
